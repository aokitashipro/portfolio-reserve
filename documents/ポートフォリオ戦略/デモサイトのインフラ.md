...
CREATE TABLE booking_reservations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES booking_users(id),
  date DATE NOT NULL,
  time TIME NOT NULL,
...
sql-- 例：booking_users テーブルのRLS設定
...
CREATE TABLE booking_users (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id TEXT NOT NULL DEFAULT 'demo-booking', -- 追加
  email TEXT NOT NULL,
  name TEXT,
  phone TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
...
-- インデックス作成（パフォーマンス向上）
CREATE INDEX idx_booking_users_tenant 
  ON booking_users(tenant_id);
...
// 使用例
// apps/demo-booking/lib/supabase.ts
import { createTenantClient } from '@/shared/supabase'

export const supabase = createTenantClient('demo-booking')
Supabase側の関数：
...
CREATE TABLE booking_reservations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id TEXT NOT NULL DEFAULT 'demo-booking',
  user_id UUID REFERENCES booking_users(id) ON DELETE CASCADE,
  date DATE NOT NULL,
  time TIME NOT NULL,
...
CREATE TABLE booking_settings (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id TEXT NOT NULL DEFAULT 'demo-booking' UNIQUE,
...
CREATE INDEX idx_booking_reservations_user ON booking_reservations(user_id);
...
CREATE POLICY "Demo users can view all restaurant data"
  ON booking_users FOR SELECT TO anon USING (true);

CREATE POLICY "Demo users can view all reservations"
  ON booking_reservations FOR SELECT TO anon USING (true);

CREATE POLICY "Demo users can view all menus"
  ON booking_menus FOR SELECT TO anon USING (true);

-- 挿入も許可（デモなので）
CREATE POLICY "Demo users can insert reservations"
  ON booking_reservations FOR INSERT TO anon WITH CHECK (true);
...
使用例
typescript// apps/demo-booking/lib/supabase.ts
...
// 使用例
const { data: reservations } = await supabase.from('booking_reservations')
// 自動的に tenant_id = 'demo-booking' でフィルタリングされる

const { data: newReservation } = await supabase.insert('booking_reservations', {
  date: '2025-01-10',
  time: '18:00',
  party_size: 4,
...
# マイグレーションファイル作成
supabase migration new create_booking_tables
...
supabase/
├── migrations/
│   ├── 20250101000000_create_booking_tables.sql
...
seed.sql（サンプルデータ）
sql-- booking_users
INSERT INTO booking_users (tenant_id, email, name, phone) VALUES
('demo-booking', 'yamada@example.com', '山田太郎', '090-1234-5678'),
('demo-booking', 'sato@example.com', '佐藤花子', '090-2345-6789'),
('demo-booking', 'tanaka@example.com', '田中一郎', '090-3456-7890');

-- booking_reservations
INSERT INTO booking_reservations (tenant_id, user_id, date, time, party_size, status)
SELECT 
  'demo-booking',
  id,
...
-- booking_menus
INSERT INTO booking_menus (tenant_id, name, description, price, category, available) VALUES
('demo-booking', '和牛ステーキ', '最高級A5ランク和牛', 5800, '肉料理', true),
('demo-booking', '海鮮パスタ', '新鮮な魚介のトマトパスタ', 1980, 'パスタ', true),
('demo-booking', 'シーザーサラダ', 'クリーミーなドレッシング', 980, 'サラダ', true),
('demo-booking', 'ティラミス', '自家製マスカルポーネ', 680, 'デザート', true);
